...
public static class DictionaryExtensions
{
		...
		this IDictionary<TKey, TValue> dictionary)
	{
		...
		// Converte sem assumir nulabilidade da chave genérica.
		var convertedDict = new Dictionary<string, object?>();

		foreach (var kvp in dictionary)
		{
			object? keyObject = kvp.Key;

			if (keyObject is null)
				throw new InvalidOperationException("Chave não pode ser null");

			var key = keyObject as string ?? keyObject.ToString()
				?? throw new InvalidOperationException("Chave não pode ser convertida para string");

			convertedDict[key] = kvp.Value;
		}
